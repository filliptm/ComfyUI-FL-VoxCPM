...
class VoxCPMPatcher(comfy.model_patcher.ModelPatcher):
    ...
    def __init__(self, model, *args, **kwargs):
        ...
        self.cache_key = f"{getattr(model, 'model_name', 'VoxCPM_Unknown')}_r{getattr(model, 'lora_rank', 32)}"
    ...
    def patch_model(self, device_to=None, *args, **kwargs):
        ...
        if self.model.model is None:
            lora_rank = getattr(self.model, 'lora_rank', 32)
            logger.info(f"Loading VoxCPM model '{self.model.model_name}' (lora_rank={lora_rank}) into RAM...")
            self.model.model = VoxCPMLoader.load_model(self.model.model_name, lora_rank=lora_rank)
