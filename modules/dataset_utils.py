...
def create_jsonl_dataset(audio_directory: str, output_filename: str = "train.jsonl") -> str:
    """
    Scans a directory for audio files (.wav, .mp3, .flac) and matching .txt transcripts,
    ...
    if not audio_files:
        raise ValueError(f"No audio files ({', '.join(SUPPORTED_AUDIO_EXTENSIONS)}) found in {audio_directory}")
    ...
    if valid_samples == 0:
        raise RuntimeError(f"No valid samples found in {audio_directory}! Ensure audio files have matching .txt transcripts.")
