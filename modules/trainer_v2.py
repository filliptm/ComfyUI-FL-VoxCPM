...
from ..src.voxcpm.model.voxcpm2 import VoxCPM2Model
...
from .trainer import send_training_update, resolve_model_path
...
def run_lora_training_v2(
    ...
    """Executes the V2 LoRA training loop."""

    torch.set_grad_enabled(True)
    logger.info(f"V2 Training - Grad Enabled: {torch.is_grad_enabled()}, Inference Mode: {torch.is_inference_mode_enabled()}")

    send_training_update(node_id, {"type": "status", "message": "Resolving V2 model path..."})
    ...
    send_training_update(node_id, {"type": "status", "message": "Loading VoxCPM V2 model..."})
    logger.info(f"Loading V2 base model from {pretrained_path}...")
    ...
    if len(trainable_params) == 0:
        ...
        if len(trainable_params) == 0:
            raise RuntimeError("Failed to enable gradients. V2 training aborted.")
    ...
    data_epoch = 0
    grad_accum_steps = train_config.get("grad_accum_steps", 8)
    ...
    with torch.enable_grad():
        for step in range(max_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with sync_context:
                    ...
                    if total_loss.grad_fn is not None:
                        ...
                        if step == 0:
                            logger.error(f"Step 0: Loss has no grad_fn!")
            ...
            if step % 10 == 0:
                ...
                print(f"[V2] Step {step}/{max_steps}, Loss: {total_loss_val:.4f}, LR: {lr_val:.8f}")
            ...
            if (step + 1) % save_every_steps == 0 or (step + 1) == max_steps:
                ...
                logger.info(f"Saved V2 checkpoint: {save_path}")
