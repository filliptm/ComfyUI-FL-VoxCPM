...
class FL_VoxCPM_TTS(io.ComfyNode):
    CATEGORY = "FL/VoxCPM"
    ...
    def define_schema(cls) -> io.Schema:
        ...
        return io.Schema(
            node_id="FL_VoxCPM_TTS",
            display_name="FL VoxCPM TTS",
            ...
            inputs=[
                ...
                io.Combo.Input("lora_name", options=lora_list, default="None", tooltip="Select a LoRA to apply for style/fine-tuning. Rank is auto-detected."),
    ...
    def execute(
        ...
        # Auto-detect LoRA rank if a LoRA is selected
        lora_rank = 32  # default
        lora_path = None
        if lora_name != "None":
            ...
            logger.info(f"Auto-detected LoRA rank: {lora_rank}")

        # Include lora_rank in cache key so models with different ranks are cached separately
        cache_key = f"{model_name}_{device}_r{lora_rank}"
        ...
        if lora_path:
            try:
                ...
                raise RuntimeError(f"Failed to load LoRA '{lora_name}'. Auto-detected rank was {lora_rank}. Error: {e}")
        else:
